...
function normaliseApplicationSteps(apiSteps, currentStatus) {
  ...
  const apiStatusByLabel = new Map(
    apiSteps.map((step) => [step.label, step.status]),
  );

  return APPLICATION_STEPS.map((label, index) => ({
    label,
    status:
      apiStatusByLabel.get(label) ??
      (index < currentIndex
        ? "done"
        : index === currentIndex
          ? "active"
...
export default function Dashboard() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    if (pendingLoanId) {
      fetch(`${API}/dashboard/sync-loan/${pendingLoanId}`, {
        method: "POST",
        headers: authHeaders(),
      })
        .then(() => {
  ...
  const markRead = async (id) => {
    try {
      ...
      setNotifications((prev) => {
        ...
        setCachedData("dashboard_notifications", updated);
    ...
    <main className="relative flex-1 overflow-y-auto pt-24 px-4 pb-4 sm:pt-24 sm:px-8 sm:pb-8 text-slate-100">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6 sm:mb-8">
        ...
        <StatCard
          ...
          label="Next EMI Due"
          value={
            summary?.next_emi_amount
              ? fmtINR(summary.next_emi_amount)
              : "No EMI"
          }
          sub={
            summary?.next_emi_due_date
              ? `● Due on ${summary.next_emi_due_date}`
              : "● No upcoming EMI"
          }
        ...
        <StatCard
          ...
          subColor={
            summary?.cibil_label === "Excellent"
              ? "text-green-400"
              : summary?.cibil_label === "Good"
                ? "text-blue-400"
                : summary?.cibil_label === "Fair"
                  ? "text-orange-400"
                  : "text-red-400"
          }
          icon={
          ...
          <div className="rounded-2xl border border-white/20 bg-white/[0.07] backdrop-blur-2xl shadow-[0_12px_32px_rgba(5,16,38,0.45),inset_0_1px_0_rgba(255,255,255,0.14)] p-4 sm:p-6">
            ...
            <div className="mt-5 flex items-center gap-2 rounded-lg bg-white/[0.06] border border-white/20 px-4 py-2.5 backdrop-blur-xl">
              ...
              <span className="text-xs text-white/50">
                {expectedDays != null
                  ? `Expected approval in ${expectedDays} day${
                      expectedDays !== 1 ? "s" : ""
                    }`
            ...
            {loadingLoans ? (
              ...
              <div className="flex flex-col gap-4">
                {loans.map((loan) => (
                  <div
                    key={loan.loan_id}
                    className="rounded-xl border border-white/20 bg-white/[0.05] backdrop-blur-xl p-4"
                  >
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-xs text-white/40">
                          {loan.repayment_percent ?? 0}% Paid (
                          {fmtINR(loan.amount_paid)})
                        </span>
                      </div>
                      ...
                      <div className="h-1.5 rounded-full bg-white/5">
                        <div
                          ...
                          style={{
                            width: `${loan.repayment_percent ?? 0}%`,
                          }}
                        />
                      </div>
                    </div>
